...
public class PlantStats
{
    ...
    // Difficulty-scaled properties
    private WorldDifficulty _currentDifficulty = WorldDifficulty.Easy;

    public void InitializeForDifficulty(WorldDifficulty difficulty)
    {
    ...
    {
        var modifiers = WorldManager.GetModifiers(WorldManager.GetCurrentWorld());

        // Scale base stats based on difficulty
        float difficultyMultiplier = GetDifficultyMultiplier(_currentDifficulty);

        // Adjust max height based on difficulty (harder = taller plants needed)
        AltezzaMassima = 5000.0f * difficultyMultiplier * modifiers.LimitMultiplier;

        // Adjust leaf count based on difficulty (harder = more leaves needed for survival)
        FoglieBase = (int)(120 * difficultyMultiplier * modifiers.LeafDropRate);

        // Adjust base resistances based on world requirements
        if (modifiers.RequiresOxygenTank)
        {
            ResistenzaVuoto = Math.Max(ResistenzaVuoto, 0.3f * difficultyMultiplier);
        }

        // Scale drop rate (harder difficulties have higher natural drop rates)
        DropRateFoglie = 0.003f * modifiers.LeafDropRate;

        // Ensure minimum values
        AltezzaMassima = Math.Max(AltezzaMassima, 2000.0f);
        FoglieBase = Math.Max(FoglieBase, 50);
    }

    private static float GetDifficultyMultiplier(WorldDifficulty difficulty) => difficulty switch
    {
        WorldDifficulty.Tutorial => 0.5f,
        WorldDifficulty.Easy => 0.8f,
        WorldDifficulty.Normal => 1.0f,
        WorldDifficulty.Medium => 1.2f,
        WorldDifficulty.MediumHard => 1.4f,
        WorldDifficulty.Hard => 1.7f,
        WorldDifficulty.VeryHard => 2.0f,
        WorldDifficulty.Extreme => 2.5f,
        WorldDifficulty.Nightmare => 3.0f,
        WorldDifficulty.Impossible => 4.0f,
        _ => 1.0f
    };

    // Dynamic property getters that apply world modifiers
    public float EffectiveMaxHeight => AltezzaMassima * WorldManager.GetCurrentModifiers().LimitMultiplier;
    public float EffectiveGrowthRate => WorldManager.GetCurrentModifiers().GrowthRateMultiplier;
    public float EffectiveHealthRegen => WorldManager.GetCurrentModifiers().HealthRegen;
    public float EffectiveWaterConsumption => WorldManager.GetCurrentModifiers().WaterConsumption;
    public float EffectiveOxygenConsumption => WorldManager.GetCurrentModifiers().OxygenConsumption;
    public float EffectiveEnergyDrain => WorldManager.GetCurrentModifiers().EnergyDrain;

    public void AggiornaTutto(DayPhase phase, Weather weather, WorldModifier worldMod)
    {
        // Apply health regeneration based on world modifiers
        float healthRegenAmount = worldMod.HealthRegen * GetPhaseMultiplier(phase) * GetWeatherMultiplier(weather);
        Salute += healthRegenAmount;

        // Apply water consumption
        float waterConsumption = worldMod.WaterConsumption * GetPhaseMultiplier(phase);
        Idratazione -= waterConsumption;

        // Apply oxygen consumption
        float oxygenConsumption = worldMod.OxygenConsumption * GetPhaseMultiplier(phase);
        Ossigeno -= oxygenConsumption;

        // Apply energy drain (metabolism)
        float energyDrain = worldMod.EnergyDrain * GetPhaseMultiplier(phase);
        Metabolismo -= energyDrain;

        // Apply temperature damage/effects
        float tempDamage = CalculateTemperatureDamage(weather, worldMod);
        Salute -= tempDamage;

        // Apply parasite effects if infested
        if (Infestata)
        {
            float parasiteDamage = worldMod.ParasiteDamage * IntensitaInfestazione;
            Salute -= parasiteDamage;
            IntensitaInfestazione += worldMod.ParasiteChance * 0.1f; // Parasites spread
        }
        else
        {
            // Chance to get infested
            if (Random.Shared.NextDouble() < worldMod.ParasiteChance)
            {
                Infestata = true;
                IntensitaInfestazione = 0.1f;
            }
        }
        ...
        {
            Salute -= worldMod.StormDamage;
        }

        // Update temperature based on weather
        Temperatura = GetWeatherTemperature(weather);

        // Handle leaf dropping based on weather and difficulty
        float leafDropChance = DropRateFoglie * GetWeatherLeafDropMultiplier(weather);
        if (Random.Shared.NextDouble() < leafDropChance && FoglieAttuali > 0)
        {
            FoglieAttuali--;
            // Add fallen leaves to compost system
            CompostSystem.AddLeaves(1, weather);
        }
    ...
    private float GetPhaseMultiplier(DayPhase phase) => phase switch
    {
        DayPhase.Morning => 1.0f,
        DayPhase.Afternoon => 1.0f,
        DayPhase.Night => 1.2f, // Night is harsher
        DayPhase.Dawn => 0.8f,
        DayPhase.Dusk => 0.9f,
        _ => 1.0f
    };

    private float GetWeatherMultiplier(Weather weather) => weather switch
    {
        Weather.Sunny => 1.0f,
        Weather.Cloudy => 1.1f,
        Weather.Rainy => 1.3f, // Rain affects water/oxygen balance
        Weather.Stormy => 1.5f,
        Weather.Windy => 1.2f,
        _ => 1.0f
    };

    private float GetWeatherLeafDropMultiplier(Weather weather) => weather switch
    {
        Weather.Sunny => 1.0f,
        Weather.Windy => 2.0f, // Wind increases leaf drop significantly
        Weather.Stormy => 3.0f, // Storms cause heavy leaf drop
        Weather.Rainy => 1.5f,
        Weather.Cloudy => 1.2f,
        _ => 1.0f
    };

    private float CalculateTemperatureDamage(Weather weather, WorldModifier worldMod)
    {
        float currentTemp = GetWeatherTemperature(weather);
        float tempDifference = Math.Abs(currentTemp - Temperatura);

        // Calculate damage based on temperature difference and resistances
        if (currentTemp < 0 && ResistenzaFreddo < tempDifference)
        {
            return (tempDifference - ResistenzaFreddo) * worldMod.TemperatureDamage;
        }
        else if (currentTemp > 30 && ResistenzaCaldo < (currentTemp - 30))
        {
            return ((currentTemp - 30) - ResistenzaCaldo) * worldMod.TemperatureDamage;
        }

        return 0f;
    }

    private float GetWeatherTemperature(Weather weather) => weather switch
    {
        Weather.Sunny => 25f,
        Weather.Cloudy => 20f,
        Weather.Rainy => 18f,
